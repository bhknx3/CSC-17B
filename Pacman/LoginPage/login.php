...
if($_SERVER["REQUEST_METHOD"] == "POST") {//Process input from user
    ...
    if(isSet($_POST["loginUsername"])) {
        ...
        $sql = "SELECT `user_id`, `user_username`, `user_password` FROM `$db`.`entity_users` WHERE `user_username`='$uname'";
        ...
        if($result->num_rows > 0) {
            ...
            $sql = "INSERT INTO `$db`.`entity_users` (`user_username`, `user_password`) VALUES ('$uname', '$pword');";
            $conn->query($sql);
            $_SESSION["username"] = $uname;
            $_SESSION["password"] = $pword;
